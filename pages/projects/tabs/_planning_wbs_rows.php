<?php
// Renders WBS rows under a SOW. Expects: $children, $wbsByParent, $id, $ganttDays, $planningSowIndex, $planningBaselineTs, $planningTotalDays, $planningBaseline (from parent).
// Optional: $planningTaskResourcesByWbsItem, $dupaItemsForPlanning for Resources section.
if (!function_exists('renderPlanningWbsRows')) {
function renderPlanningWbsRows(array $children, array $wbsByParent, int $projectId, array $ganttDays, int $depth = 1, $sowIndex = null, $baselineTs = null, $totalDays = 1, $baselineYmd = '', array $resourcesByWbsItem = [], array $dupaItems = [], $colspan = 0, $redirectHidden = '', $baseUrl = '', array $durationFromResources = []): void {
    ...
    foreach ($children as $w) {
        ...
        $resourceDur = (int)($durationFromResources[$wbsId] ?? 0);
        $plannedDur = $resourceDur > 0 ? $resourceDur : ($storedDur > 0 ? $storedDur : '');
        ...
        $formId = 'wbs-form-' . $wbsId;
        $taskResources = $resourcesByWbsItem[$wbsId] ?? [];
        $resourceCount = count($taskResources);
        $showResourcesSection = $colspan > 0;
        ?>
        <tr class="wbs-row planned-row" data-wbs-id="<?= $wbsId ?>">
            <td class="col-id text-center">
                <form id="<?= $formId ?>" action="<?= $baseUrl ?>/actions/project_actions.php" method="POST" class="d-none">
                    <input type="hidden" name="action" value="update_wbs_planned_dates">
                    <input type="hidden" name="project_id" value="<?= $projectId ?>">
                    <input type="hidden" name="wbs_id" value="<?= $wbsId ?>">
                    <?= $redirectHidden ?>
                    <input type="hidden" name="planned_start_date" id="wbs-form-start-<?= $wbsId ?>" value="<?= htmlspecialchars($plannedStart, ENT_QUOTES, 'UTF-8') ?>">
                    <input type="hidden" name="planned_end_date" id="wbs-form-end-<?= $wbsId ?>" value="<?= htmlspecialchars($plannedEnd, ENT_QUOTES, 'UTF-8') ?>">
                </form>
            </td>
            <td class="col-desc planning-wbs-desc-cell" style="padding-left:<?= $pad ?>px">
                <div class="planning-activity-cell">
                    <div class="d-flex align-items-center planning-activity-row">
                        <form id="wbs-edit-form-<?= $wbsId ?>" action="<?= $baseUrl ?>/actions/project_actions.php" method="POST" class="planning-wbs-name-form d-flex align-items-center flex-grow-1 min-w-0">
                            <input type="hidden" name="action" value="update_wbs_item">
                            <input type="hidden" name="project_id" value="<?= $projectId ?>">
                            <input type="hidden" name="wbs_id" value="<?= $wbsId ?>">
                            <?= $redirectHidden ?>
                            <input type="text" name="code" class="form-control form-control-sm wbs-code-input planning-code-input" value="<?= htmlspecialchars($displayCode, ENT_QUOTES, 'UTF-8') ?>" placeholder="<?= $sowIndex !== null && $depth === 1 ? $sowIndex . '.1' : '1.1' ?>" title="Code (e.g. 1.1, 2.1)">
                            <input type="text" name="description" class="form-control form-control-sm wbs-desc-input" value="<?= htmlspecialchars($w['description'] ?? '', ENT_QUOTES, 'UTF-8') ?>" placeholder="Activity name" title="Rename activity">
                        </form>
                        <span class="planning-row-actions">
                            <button type="button" class="btn btn-outline-secondary btn-sm py-0 px-1 me-1 wbs-save-all-btn" title="Save name and dates" data-name-form-id="wbs-edit-form-<?= $wbsId ?>" data-dates-form-id="<?= $formId ?>">Save</button>
                            <form action="<?= $baseUrl ?>/actions/project_actions.php" method="POST" class="d-inline" onsubmit="if(typeof window.saveViewScroll==='function') window.saveViewScroll(); return confirm('Delete this work breakdown item?');">
                                <input type="hidden" name="action" value="delete_wbs_item">
                                <input type="hidden" name="project_id" value="<?= $projectId ?>">
                                <input type="hidden" name="wbs_id" value="<?= $wbsId ?>">
                                <?= $redirectHidden ?>
                    ...
                    <div class="planning-resources-row-wrap d-flex align-items-center gap-2">
                        ...
                        <button type="button" class="btn btn-outline-primary btn-sm planning-resources-toggle flex-grow-1 text-start" data-target-id="planning-resources-wbs-<?= $wbsId ?>" title="Toggle resources">
            ...
            <td class="col-duration">
                <input type="number" form="<?= $formId ?>" name="planned_duration_days" class="form-control form-control-sm wbs-duration-input text-center" min="1" max="365" value="<?= $plannedDur !== '' ? $plannedDur : '' ?>" placeholder="—" style="max-width:5rem">
            </td>
            <td class="col-start">
                <input type="number" class="form-control form-control-sm wbs-day-start-input text-center" min="1" max="<?= (int)$totalDays ?>" value="<?= $plannedStartDay !== null ? (int)$plannedStartDay : '' ?>" placeholder="—" style="max-width:4.5rem" data-date-id="wbs-form-start-<?= $wbsId ?>" data-form-id="<?= $formId ?>" data-baseline="<?= htmlspecialchars($baselineYmd, ENT_QUOTES, 'UTF-8') ?>" title="Start day">
            </td>
            <td class="col-end">
                <span class="wbs-end-day-display text-muted">Day <?= $plannedEndDay !== null ? (int)$plannedEndDay : '—' ?></span>
            </td>
            <?php
        ...
        <?php if ($showResourcesSection): ?>
        <?php
        $wbsBudgetItemId = isset($w['budget_item_id']) && $w['budget_item_id'] !== '' && $w['budget_item_id'] !== null ? (int)$w['budget_item_id'] : null;
        $dupaForThisSow = [];
        ...
        ?>
        <tr id="planning-resources-wbs-<?= $wbsId ?>" class="planning-resources-row table-light" style="display:none" data-planning-pad="<?= (int)$pad ?>">
                ...
                <div class="planning-resources-vertical d-flex flex-column gap-2">
                    <div class="border rounded p-2 bg-white">
                        <h6 class="small text-uppercase text-muted mb-2"><i class="bi bi-people me-1"></i>Manpower</h6>
                            ...
                            <form action="<?= $baseUrl ?>/actions/project_actions.php" method="POST" class="d-flex flex-wrap align-items-end gap-1 small" onsubmit="if(typeof window.saveViewScroll==='function') window.saveViewScroll();">
                                <input type="hidden" name="action" value="add_planning_task_resource"><input type="hidden" name="project_id" value="<?= $projectId ?>"><input type="hidden" name="task_type" value="wbs_item"><input type="hidden" name="wbs_item_id" value="<?= $wbsId ?>"><?= $redirectHidden ?>
                    ...
                    <div class="border rounded p-2 bg-white">
                        <h6 class="small text-uppercase text-muted mb-2"><i class="bi bi-truck me-1"></i>Equipment</h6>
                            ...
                            <form action="<?= $baseUrl ?>/actions/project_actions.php" method="POST" class="d-flex flex-wrap align-items-end gap-1 small" onsubmit="if(typeof window.saveViewScroll==='function') window.saveViewScroll();">
                                <input type="hidden" name="action" value="add_planning_task_resource"><input type="hidden" name="project_id" value="<?= $projectId ?>"><input type="hidden" name="task_type" value="wbs_item"><input type="hidden" name="wbs_item_id" value="<?= $wbsId ?>"><?= $redirectHidden ?>
                                <select name="dupa_item_id" class="form-select form-select-sm" style="min-width:140px"><option value="">— select —</option><?php foreach ($dupaForThisSow['equipment'] ?? [] as $d): ?><option value="<?= (int)$d['id'] ?>"><?= htmlspecialchars($d['description'], ENT_QUOTES, 'UTF-8') ?></option><?php endforeach; ?></select>
                                <input type="number" name="quantity" class="form-control form-control-sm" value="1" min="0.001" step="any" style="width:55px" placeholder="Qty">
                                <input type="text" name="unit" class="form-control form-control-sm" placeholder="Unit" style="width:50px">
                                <select name="use_rule" class="form-select form-select-sm planning-use-rule-select"><option value="spread">Spread</option><option value="start">Start</option><option value="end">End</option></select>
                                <button type="submit" class="btn btn-primary btn-sm">Add</button>
                            </form>
                    </div>
        ...
        <?php endif; ?>
        <?php
        $sub = $wbsByParent[$wbsId] ?? [];
        if (!empty($sub)) {
            renderPlanningWbsRows($sub, $wbsByParent, $projectId, $ganttDays, $depth + 1, null, $baselineTs, $totalDays, $baselineYmd, $resourcesByWbsItem, $dupaItems, $colspan, $redirectHidden, $baseUrl, $durationFromResources);
        }
    }
}
}
if (!empty($children)) {
    ...
    renderPlanningWbsRows($children, $wbsByParent, (int)$id, $ganttDays, 1, $sowIndex, $baselineTs, $totalDays, $baselineYmd, isset($planningTaskResourcesByWbsItem) ? $planningTaskResourcesByWbsItem : [], isset($dupaItemsForPlanning) ? $dupaItemsForPlanning : [], $colspan, $redirectHidden, defined('BASE_URL') ? BASE_URL : '', isset($planningDurationFromResourcesByWbsItem) ? $planningDurationFromResourcesByWbsItem : []);
}
